...
public class App {
	...
	public CommandLineRunner demo(EmployeeRepository employeeRepository) {
		return (args) -> {
			...
			System.out.println("Get all the employees whose id is in 2, 3, 5\n");
			List<Employee> emps = employeeRepository.findByIdIn(Arrays.asList(2, 3, 5));
			emps.stream().forEach(System.out::println);
			
			System.out.println("\n\n");
			emps = employeeRepository.findByIdIsIn(Arrays.asList(2, 3, 5));
			emps.stream().forEach(System.out::println);

			System.out.println("\n\n");
			emps = employeeRepository.getEmpsByIds(Arrays.asList(2, 3, 5));
			emps.stream().forEach(System.out::println);
			
			System.out.println("\n\n");
			emps = employeeRepository.getEmpsByIdsUsingNativeQuery(Arrays.asList(2, 3, 5));
			emps.stream().forEach(System.out::println);
