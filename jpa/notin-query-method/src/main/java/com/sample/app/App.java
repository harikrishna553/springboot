...
public class App {
	...
	public CommandLineRunner demo(EmployeeRepository employeeRepository) {
		return (args) -> {
			...
			emps = employeeRepository.findByIdNotIn(Arrays.asList(2, 3, 5));
			printEmployees(emps, "Employees with id not in (2, 3, 5)");

			emps = employeeRepository.findByIdNotInOrFirstNameNotIn(Arrays.asList(2, 3), Arrays.asList("Ram"));
			printEmployees(emps, "Employees where id is not in [2, 3] or firstName not in ['Ram']");

			emps = employeeRepository.findByIdNotInAndFirstNameIn(Arrays.asList(2, 3), Arrays.asList("Ram"));
			printEmployees(emps, "Employees where id is not in [2, 3] and firstName in ['Ram']");
		};
	}
